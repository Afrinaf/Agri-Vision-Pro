...
const ChatState = ({ children, userId }) => {
    ...
    const fetchChats = async () => {
        try {
            const res = await fetch("http://localhost:5005/api/chat/list", {
                headers: {
    ...
    const fetchMessages = async (postId, bidId) => {
        try {
            const res = await fetch(`http://localhost:5005/api/chat/fetch/${postId}/${bidId}`, {
                headers: {
                    "auth-token" : localStorage.getItem("token")
                },
            });
    ...
    const sendMessage = async (messageText) => {
        ...
        try {
            ...
            const res = await fetch("http://localhost:5005/api/chat/send", {
                method: "POST",
                headers: {
