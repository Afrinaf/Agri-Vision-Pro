...
const BidState = (props) => {
    ...
    const placeBid = async (postId) => {
        try {
            const response = await fetch('http://localhost:5005/api/bid/placebid', {
                method: 'PUT',
                headers: {
    ...
    const getBid = async (postId) => {
        try {
            const response = await fetch('http://localhost:5005/api/bid/getpostbid', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'auth-token': localStorage.getItem('token')
    ...
    const acceptBid = async (bidId) => {
        try {
            const res = await fetch("http://localhost:5005/api/bid/acceptbid", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    'auth-token': localStorage.getItem('token')
    ...
    const rejectBid = async (bidId) => {
        try {
            const res = await fetch("http://localhost:5005/api/bid/rejectbid", {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                    'auth-token': localStorage.getItem('token')
    ...
    const fetchUserBids = async (userId) => {
        try {
            const response = await fetch("http://localhost:5005/api/bid/getuserbids", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "auth-token": localStorage.getItem("token")
